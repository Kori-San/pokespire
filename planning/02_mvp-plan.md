...
- **Data layer — self-host decision.** No runtime PokéAPI. `scripts/fetch-pokedex.mjs` (committed) vendors Gen 1 data + BW-animated sprites locally (offline-first, no rate limits, one pipeline; Gen 6+ via Smogon later). All-gens **multi-form schema**: species hold `forms[]` (each with own `formId`/types/stats/sprites) + generation, growth/catch rate, `genderRate`, `hasGenderDifferences`, `evolvesFrom`, `isLegendary/isMythical/isParadox/hasMega/hasGmax/soloOnly`. Sprites keyed by `formId` so regional/mega/Gmax coexist.
- **Solo-only = legendary/mythical/paradox SPECIES only.** Mega/Gmax are **in-combat transform cards** (Mega Evolve card; Dynamax card → Gmax form if one exists) — any team mon can use them, never gating to solo.
- **Built & committed:** the vendoring **script** (the generated `pokedex.gen1.ts` + `public/sprites/**` are produced locally but **not vendored into git yet** — deferred by choice). MissingNo GIF is the missing-sprite fallback.
- **Local-only / uncommitted now:** generated data + sprites; `services/sprites.ts` (local form resolver); UI primitives `HpBar`/`TypeChip` (+ stories); a first-pass `Card`.
- **Stopping here:** the **`Card` component needs a redo** (per the user). Deferred: sprite fetch → **Gen-V-animated-only** (drop static fallback); single-frame "gif" forms (e.g. `articuno-galar`) need detection.
- **Next session:** redo `Card`, then `Hand` / `PartyBar` / `IntentBadge` / `BattleStage` / CombatScreen (switch + end-of-demo overlay) + stories; decide when to vendor+commit data/sprites. SpeciesDef-in-combat deferred to run-loop.
